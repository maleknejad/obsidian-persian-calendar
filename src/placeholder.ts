	...
	public async insertPersianDate(file: TFile): Promise<void> {
		...
		setTimeout(async () => {
			...
			const placeholders: { [key: string]: PlaceholderValue } = {
				...
				"{{این روز}}": this.getFormattedDateFromFileTitle(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{روز هفته}}": this.getCurrentWeekday(),
				"{{این روز هفته}}": this.getWeekdayFromFileTitle(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{هفته}}": this.getCurrentWeek(),
				"{{این هفته}}": this.getWeekNumberFromFileTitle(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{ماه}}": this.getCurrentMonth(),
				"{{این ماه}}": this.getMonthNumberFromFileTitle(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{فصل}}": this.getCurrentQuarter(),
				"{{این فصل}}": this.getQuarterNumberFromFileTitle(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{سال}}": this.getCurrentYear(),
				"{{این سال}}": this.getYearNumberFromFileTitle(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{روزهای گذشته}}": this.getDaysPassedFromFileTitle(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{روزهای باقیمانده}}": this.getDaysUntilEndOfYear(
					file.basename,
					this.plugin.settings.dateFormat,
				),
				"{{اول هفته}}": this.isWeeklyFile(file.basename)
					? this.getWeekStartDate(
							parseInt(file.basename.split("-W")[0]),
							parseInt(file.basename.split("-W")[1]),
							this.plugin.settings.dateFormat,
					  )
					: null,
				"{{آخر هفته}}": this.isWeeklyFile(file.basename)
					? this.getWeekEndDate(
							parseInt(file.basename.split("-W")[0]),
							parseInt(file.basename.split("-W")[1]),
							this.plugin.settings.dateFormat,
					...
					: null,
				"{{اول ماه}}": this.isMonthlyFile(file.basename)
					? this.getMonthStartDate(file.basename, this.plugin.settings.dateFormat)
					: null,
				"{{آخر ماه}}": this.isMonthlyFile(file.basename)
					? this.getMonthEndDate(file.basename, this.plugin.settings.dateFormat)
					: null,
